...
namespace tinylang{
    ...
    void CGProcedure::writeVariable(llvm::BasicBlock* BB, Decl* D, llvm::Value* Val){
        if(auto* V = llvm::dyn_cast<VariableDeclaration>(D)){
            if(V->getEnclosingDecl() == Proc){
                ...
                llvm::report_fatal_error("Nested functions not supported");
            }
    ...
    llvm::Function* CGProcedure::createFunction(ProcedureDeclaration* Proc, llvm::FunctionType* FTy){
        ...
        llvm::Function* Fn = llvm::Function::Create(Fty, llvm::GlobalValue::ExternalLinkage,
    ...
    void CGProcedure::emitStmt(IfStatement* Stmt){
        ...
        llvm::BasicBlock* ElseBB = HasElse ? llvm::BasicBlock::Create(CGM.getLLVMCtx(), "else.body", Fn)
                                           :nullptr;
