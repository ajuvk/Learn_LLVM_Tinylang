...
void Sema::checkFormalAndActualParameters(SMLoc Loc, const FormalParamList& Formals,
...
void Sema::actOnFormalParameterDeclaration(FormalParamList& Params, IdentList& Ids, Decl* D, bool IsVar){
    ...
    if (TypeDeclaration* Ty = dyn_cast<TypeDeclaration>(D)){
        for(auto & [Loc, Name]: Ids){
            FormalParameterDeclaration* Decl = new FormalParameterDeclaration(
...
void Sema::actOnProcedureHeading(ProcedureDeclaration* ProcDecl, FormalParamList& Params,
        Decl* RetType){
...
void Sema::actOnIfStatement(StmtList& Stmts, SMLoc Loc, Expr* Cond, StmtList& IfStmts,
                            StmtList& ElseStmts){
    if(!Cond){
        Cond = FalseLiteral;
    }
...
Expr* Sema::actOnExpression(Expr* Left, Expr* Right, const OperatorInfo& Op){
    ...
    if (Left->getType() != Right->getType()) {
        Diags.report(
          Op.getLocation(),diag::err_types_for_operator_not_compatible,
